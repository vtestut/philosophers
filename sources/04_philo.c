...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/03 18:24:18 by vtestut           #+#    #+#             */
/*   Updated: 2023/08/26 02:13:58 by vtestut          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	eat(t_philo *philo)
{
	...
	print_state(philo, false, FORK_1);
	pthread_mutex_lock(&philo->d->forks_mtx[philo->fork[1]]);
	print_state(philo, false, FORK_2);
...
static void	think(t_philo *philo, bool silent)
{
	...
	pthread_mutex_lock(&philo->meal_time_mtx);
	tt_think = (philo->d->tt_die
			- (get_time() - philo->last_meal) - philo->d->tt_eat) / 2;
	pthread_mutex_unlock(&philo->meal_time_mtx);
	if (tt_think < 0)
		tt_think = 0;
